...
async function handleReceiptUpload(req, res, next) {
    ...
    try {
        ...
        const MAX_PROCESSING_TIME = Number(process.env.RECEIPT_MAX_PROCESSING_MS) || 120000;
        const processingPromise = processReceiptUpload(file.buffer, file.mimetype, billingId, sheetsClient, requestId);
        const timeoutPromise = new Promise((_, reject) => {
            setTimeout(() => {
                reject(createError('Scan took too long. Try again later or with a smaller/clearer image.', 504, 'SCAN_TIMEOUT'));
            }, MAX_PROCESSING_TIME);
        });
