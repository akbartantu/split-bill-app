    ...
    try {
        ...
        const response = await client.chat.completions.create({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are an OCR engine for receipts. Transcribe all visible text as accurately as possible. Return ONLY the raw text, preserving line breaks. Do not add explanations, labels, or formatting.',
                },
                {
                    role: 'user',
                    content: [
        ...
        if (!content) {
            throw createError('OpenAI OCR returned empty text', 500, 'OPENAI_OCR_EMPTY');
        }
        return {
            text: content,
