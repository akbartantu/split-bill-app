...
export async function runOCR(imageBuffer, options = {}) {
    ...
    try {
        if (provider === 'tesseract') {
            ...
            return await runOpenAIOCR(imageBuffer, timeout);
        }
        else if (provider === 'google-vision') {
            return await runGoogleVisionOCR(imageBuffer, timeout);
        }
        else {
            throw createError(`Unsupported OCR provider: ${provider}`, 400, 'UNSUPPORTED_OCR_PROVIDER');
        }
    }
    catch (error) {
        // Re-throw API errors
        ...
        // Wrap other errors
        throw createError(`OCR failed: ${error.message}`, 500, 'OCR_ERROR', { originalError: error.message });
    }
}
/**
 * Run Tesseract OCR with timeout
 */
async function runTesseractOCR(imageBuffer, timeout) {
    return new Promise(async (resolve, reject) => {
        // Set timeout
        const timeoutId = setTimeout(() => {
            reject(createError('OCR timeout - processing took too long (exceeded 30 seconds)', 408, 'OCR_TIMEOUT'));
        }, timeout);
        try {
            // Validate buffer
            ...
            // Wrap other errors
            reject(createError(`OCR execution failed: ${error.message}`, 500, 'OCR_EXECUTION_ERROR'));
        }
    });
}
/**
 ...
 */
async function runGoogleVisionOCR(imageBuffer, timeout) {
    // TODO: Implement Google Vision OCR
    throw createError('Google Vision OCR not yet implemented', 501, 'NOT_IMPLEMENTED');
}
