 ...
 */
export function reconstructReceiptLine(normalizedLine, originalLine, opts) {
    const skip = (reason) => {
    ...
    if (prices.length === 0) {
        ...
        let nameOnly = line;
        if (qtyMatch)
            nameOnly = nameOnly.replace(qtyMatch[0], '').trim();
        nameOnly = extractCanonicalName(nameOnly);
        // Skip receipt-header-like names (no-price only)
        const trimmedName = nameOnly.trim();
        if (/^(KURANDA|RAINFOREST|RESTAURANT|VIEW|RAINFOREST VIEW)$/i.test(trimmedName))
            return skip('header');
        if (/COONDOO/i.test(nameOnly) || /COONDOO/i.test(line))
            return skip('header');
        if (/\d+\s*ST\s/.test(line) && /(QLD|AUST)/i.test(line))
            return skip('header');
        if (/\bCLERK\b/i.test(nameOnly))
            return skip('header');
        // Skip total-like and OCR-garbage names (no-price only)
        if (/^(WAAL|TOTAL|GST|NET|BAL|BALANCE|OTAL)$/i.test(trimmedName))
            return skip('total/garbage');
        if ((nameOnly.match(/\d/g) || []).length >= 2)
            return skip('total/garbage');
        ...
        if (tokens.length >= 4)
            return skip('total/garbage');
        if (nameOnly.length < 2 || nameOnly.length > 100)
            return skip('no price');
        const garbageNamePatternsNoPrice = [
            /^Snerrr\s*\d*$/i,
            /^[A-Z]{1,2}\s+\d\s*$/,
            /^ST\s+5\s+6ST$/i,
            /^GT\s*ST\s*SS?\.?$/i,
        ...
        if (garbageNamePatternsNoPrice.some(p => p.test(nameOnly)))
            return skip('no price');
        if (nameOnly.length <= 12 && !/[aeiou]/i.test(nameOnly))
            return skip('no price');
        return {
            quantity,
            itemName: nameOnly,
    ...
    if (prices.length === 1) {
        ...
        if (quantity > 1) {
            ...
            if (difference > tolerance) {
                reviewReasons.push(`Quantity mismatch: ${quantity} × $${unitPrice.toFixed(2)} = $${expectedTotal.toFixed(2)}, but line total is $${lineTotal.toFixed(2)}`);
