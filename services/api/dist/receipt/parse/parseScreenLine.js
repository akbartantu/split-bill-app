...
export function isNonItemLine(line) {
    return nonItemPatterns.some(p => p.test(line));
}
export function extractExtrasAmount(line) {
    if (!extrasPattern.test(line))
        return null;
...
export function shouldAppendToPreviousName(line) {
    if (/\d/.test(line))
        return false;
    if (/[.,]$/.test(line))
        return false;
...
export function parseScreenLine(normalizedLine) {
    if (normalizedLine.length < 3)
        return null;
    if (isNonItemLine(normalizedLine))
        return null;
