 ...
 */
export function parseReceipt(ocrText, ocrConfidence) {
    try {
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                if (sanityResult.isSuspicious && item.totalPrice > 0) {
                    ...
                    if (correction.corrected && correction.confidence > 0.6) {
                        ...
                        item.reviewReasons.push(`Auto-corrected ${correction.correctionType}: ${oldTotal.toFixed(2)} → ${correction.correctedValue.toFixed(2)}`);
                    }
                }
            }
        }
