 ...
 */
export async function processReceiptUpload(imageBuffer, mimetype, billingId, sheetsClient, requestId) {
    // Use new receipt-aware pipeline
    try {
        const scanResult = await scanReceipt(imageBuffer, mimetype, billingId, sheetsClient, requestId);
        return {
    ...
    return processReceiptUploadLegacy(imageBuffer, mimetype, billingId, sheetsClient, requestId);
}
/**
 * Legacy receipt processing (fallback)
 */
async function processReceiptUploadLegacy(imageBuffer, mimetype, billingId, sheetsClient, requestId) {
    ...
    try {
        ...
        try {
            ...
            throw createError(`Image preprocessing failed: ${error.message}`, 400, 'PREPROCESSING_ERROR');
        }
        // Step 3: Run OCR
        ...
        try {
            ...
            throw createError(`OCR failed: ${error.message}`, 500, 'OCR_ERROR');
        }
        ...
        return {
            ...
            receipt: {
                ...
                needsReview: parsed.items.some((item) => item.needsReview) || parsed.items.length === 0,
            },
        ...
        throw createError(`Receipt processing failed: ${error.message}`, 500, 'RECEIPT_PROCESSING_ERROR', { originalError: error.message });
    }
}
/**
 * Save receipt to spreadsheet
 */
async function saveReceiptToSheets(client, billingId, parsed) {
    try {
        ...
        if (!spreadsheetId || spreadsheetId === 'replace_me' || spreadsheetId.trim() === '') {
            throw createError('Spreadsheet not configured. Set SPREADSHEET_ID in .env', 500, 'SPREADSHEET_NOT_CONFIGURED');
        }
        // Validate billing ID
        ...
        throw createError(`Failed to save receipt to spreadsheet: ${error.message}`, 502, 'SPREADSHEET_ERROR');
    }
}
function generateId() {
