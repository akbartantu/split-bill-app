 ...
 */
export async function preprocessImage(buffer, mimetype) {
    // Validate input
    ...
    try {
        ...
        try {
            ...
            throw createError(`Failed to read image metadata: ${error.message}`, 400, 'INVALID_IMAGE');
        }
        ...
        if (processedBuffer.length > MAX_SIZE_AFTER_PROCESSING) {
            throw createError(`Processed image still too large (${processedBuffer.length} bytes). Please use a smaller image.`, 400, 'IMAGE_TOO_LARGE');
        }
        return {
 ...
 */
function preprocessImageBasic(buffer, mimetype) {
    // Basic validation
    if (buffer.length === 0) {
        throw createError('Image buffer is empty', 400, 'EMPTY_IMAGE');
    }
    if (buffer.length > MAX_SIZE_AFTER_PROCESSING) {
        throw createError(`Image too large (${(buffer.length / 1024 / 1024).toFixed(1)}MB). Maximum: ${(MAX_SIZE_AFTER_PROCESSING / 1024 / 1024).toFixed(1)}MB`, 400, 'IMAGE_TOO_LARGE');
    }
    // Validate image format by checking magic bytes
    const isValidImage = (mimetype === 'image/jpeg' && buffer[0] === 0xFF && buffer[1] === 0xD8) ||
