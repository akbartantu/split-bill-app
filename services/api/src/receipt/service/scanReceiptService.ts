...
/** API response shape for a single item (camelCase for frontend) */
export interface ScanReceiptItemResponse {
  ...
  totalPrice: number;
  /** Allocated share of receipt tax for this item (proportional to line total). null if no tax on receipt. */
  ...
  try {
    ...
    if (parserProvider === 'openai') {
      try {
        ...
        const items: CanonicalReceiptItem[] = (aiParsed.items || []).map(item => ({
          item_id: generateId(),
        ...
        const overallConfidence = Math.min(1, (
          avgItemConfidence * 0.6 +
          (items.length > 0 ? 0.2 : 0) +
          (hasTotal ? 0.1 : 0) +
          (totalMatches ? 0.1 : 0)
        ));

        const allocateTax = (lineTotal: number): number | null => {
          const tax = aiParsed.tax ?? null;
          if (tax == null || tax === 0 || itemsSum <= 0 || lineTotal <= 0) return null;
    ...
    const hasPriceInLine = (s: string) => /\d{1,3}[.,]\d{2}/.test(s);
    const isNameOnly = (s: string) => {
    ...
    const hasLasagna2095 = (s: string) => /LASANGA|LASAGNA/i.test(s) && /\b20\.95\b/.test(s);
    const hasChickenOpenNoPrice = (s: string) => /CHICKEN\s+OPEN/i.test(s) && !/\d{1,3}[.,]\d{2}/.test(s);
