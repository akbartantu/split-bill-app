...
/**
 * Semantic representation of a single line item on the receipt.
 *
 * This sits at the end of the pipeline after:
 * 1) Text detection + preprocessing (Sharp / document detection)
 * 2) Text recognition (OpenAI vision → raw OCR text)
 * 3) Layout understanding (grouping lines into items/summary)
 */
...
/**
 * Semantic representation of the whole receipt produced by OpenAI.
 *
 * We keep the summary flexible so we can support different tax schemes:
 * - subtotal: pre‑tax amount
 * - tax: generic tax
 * - gst: GST specifically (if present)
 * - total: grand total
 * - totalPaid: optional alias for total_paid
 */
...
export async function parseReceiptWithOpenAI(
  ...
  try {
     ...
     */
    const callOnce = async (purpose: 'initial' | 'correction', previousJson?: AiParsedReceipt) => {
      const systemPurpose =
        purpose === 'initial'
...
`;
      ...
      if (!content) {
        throw createError(
          'OpenAI parser returned empty content',
          500,
          'OPENAI_PARSE_EMPTY'
        );
      }

      const parsed = JSON.parse(content) as AiParsedReceipt;

      if (!parsed.items || !Array.isArray(parsed.items)) {
        throw createError(
          'OpenAI parser returned invalid structure (missing items array)',
          500,
          'OPENAI_PARSE_INVALID'
        );
      }
    ...
    throw createError(
      `OpenAI receipt parsing failed: ${error.message || String(error)}`,
      500,
      'OPENAI_PARSE_ERROR',
...
export function validateReceiptTotals(
  receipt: AiParsedReceipt,
  tolerance: number = 0.05
): { isValid: boolean; difference: number; shouldRequery: boolean } {
