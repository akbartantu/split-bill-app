...
import type { ScanReceiptResult, ParsedReceipt } from '@/lib/ocr';
...
export function ReceiptScanner({ open, onClose, onComplete, targetReceiptId, currencyCode = 'USD' }: ReceiptScannerProps) {
  ...
  // Track latest progress in a ref so interval callbacks always see
  // the most recent value instead of a stale closure snapshot.
    ...
    if (processingState === 'processing' || processingState === 'uploading') {
      progressCheckRef.current = setInterval(() => {
        ...
        if (current === last && current < 100) {
          console.warn(
            `[ReceiptScanner] [${requestId}] Progress stuck at ${current}%`
          );
        }
  ...
  const processFile = async (file: File) => {
    ...
    // On file selection: show small initial progress
    setProgress(5);
    ...
    // Simulated progress timer while waiting for API response
    let simulatedTimer: ReturnType<typeof setInterval> | null = null;
    const clearSimulatedTimer = () => {
      if (simulatedTimer) {
    ...
    // This will hold the normalized scan result once the API call succeeds.
    let scanResult: ScanReceiptResult | null = null;
    ...
    try {
      try {
        ...
        // On upload start: set progress to 20 and begin simulated ramp to 90
        setProgress(20);
        ...
        const apiResult = await uploadReceiptImage(
          ...
          reqId,
          (stage /* , prog */) => {
            // We still use the stage labels for UI text,
            // but the numeric progress comes from the simulated timer.
        ...
        // Response arrived — stop simulated progress
        clearSimulatedTimer();
        
        // Defensive checks on API response shape:
        // - Only require that it's a non-null object.
        // - Do NOT require items.length > 0.
        // - If items are missing, normalize to [].
        if (apiResult == null || typeof apiResult !== 'object') {
          throw new Error('Scan failed: invalid response from server.');
        }

        const rawReceipt: any = (apiResult as any).receipt ?? {};
        const rawItems: any[] = Array.isArray(rawReceipt.items)
          ? rawReceipt.items
          : [];

        // Normalize API response into ScanReceiptResult format
        scanResult = {
          ...
          receipt: {
            ...
            confidence: rawReceipt.confidence ?? 0,
            rawText: '',
            subtotal:
              (apiResult as any).subtotal ??
              rawReceipt.subtotal ??
              undefined,
            tax:
              (apiResult as any).tax ??
              rawReceipt.tax ??
              undefined,
            total:
              (apiResult as any).total ??
              rawReceipt.total ??
              undefined,
          },
          // Empty items are allowed; we may still suggest manual entry for UX
          needsManualEntry:
            rawReceipt.needsReview || rawItems.length === 0,
          rawOCRText: '',
          documentDetected:
            (apiResult as any).documentDetected ??
            rawReceipt.documentDetected,
          detectionStrategy:
            (apiResult as any).detectionStrategy ??
            rawReceipt.detectionStrategy,
        };
        ...
        // Surface API/server error directly; do not fallback to client-side OCR
        throw new Error(apiError.message || 'Scan failed via API');
      }
      
      // Ensure we have a normalized result before proceeding
      if (!scanResult) {
        throw new Error('Scan failed: no result returned from server.');
      }
      ...
      if (import.meta.env.DEV) {
        console.log(`[ReceiptScanner] [${reqId}] Scan completed`, {
          ...
          itemCount: scanResult.receipt.items.length,
      ...
      if (scanResult.needsManualEntry && scanResult.receipt.items.length === 0) {
        setManualEntryText(scanResult.rawOCRText || '');
        setShowManualEntry(true);
      } else {
        setShowManualEntry(false);
      }
      
      // Completed successfully: set progress to 100
      setProgress(100);
      setStage('review');
    } catch (err: any) {
      clearSimulatedTimer();
      ...
      if (err.name === 'AbortError') {
        ...
        errorMessage = 'Scan took too long (time limit). Try a clearer image or use manual input.';
        ...
        errorMessage = 'Server took too long to process. Please try again or use a smaller image.';
      }
                ...
                <div className="space-y-2">
                  ...
                  {apiCheckFailed && (
                    <p className="text-xs text-muted-foreground">
                      API is not reachable. Please make sure the server is running and try again.
                    </p>
                  )}
                </div>
              )}
